...
| Analysis | Main context | Background agent (default model) |
...
| Feature | Description |
|---------|-------------|
| Cross-platform | Node.js scripts (.js) instead of Bash (.sh) |
| Silent execution | `windowsHide: true` - no flashing windows |
| Auto-start | SessionStart hook auto-starts observer |
| Default model | Empty model config uses Claude Code default |

## What's New in v2.2

| Feature | Description |
|---------|-------------|
| Observer writes files | No permission issues - observer process creates instinct files |
| Limited analysis | Only analyzes latest 50 observations to avoid token limits |
| Output format | Uses `FILE: name.md` ... `END` format for file creation |
| Max turns | Increased to 10 turns for complex pattern analysis |
...
```json
{
  "hooks": {
    ...
    "SessionStart": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "node \"${CLAUDE_PLUGIN_ROOT}/skills/continuous-learning-v2/agents/start-observer.js\" start"
      }]
    }]
  }
}
...
**Note:** The hooks use `.js` files (Node.js) instead of `.sh` for Windows compatibility. Both `observe.js` and `start-observer.js` run silently without flashing windows.
...
```json
{
  ...
  "observer": {
    ...
    "max_observations_per_analysis": 50,
    "patterns_to_detect": [
      "user_corrections",
      "error_resolutions",
      "repeated_workflows",
...
├── observer.log           # Observer analysis logs
├── .observer.pid          # Observer process ID (internal)
├── instincts/
...
The observer can be manually controlled:

```bash
# Start observer (usually automatic via SessionStart hook)
node start-observer.js start

# Check observer status
node start-observer.js status

# Stop observer
node start-observer.js stop
```
